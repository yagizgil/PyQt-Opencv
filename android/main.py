...
class Window(QMainWindow):
    ...
    def setUI(self):
        ...
        self.w = QSpinBox()
        self.w.setMaximum(5000)
        self.w.setValue(640)
        self.w.valueChanged.connect(self.set_re)
        
        self.h = QSpinBox()
        self.h.setMaximum(5000)
        self.h.setValue(480)
        self.h.valueChanged.connect(self.set_re)
        
        h.addWidget(self.w)
        h.addWidget(self.h)
        f.addRow(h)
        
        self.ul = QLineEdit("http://192.168.1.31:8080//shot.jpg")
        self.ul.setPlaceholderText("url")
        f.addRow(self.ul)
    ...
    def set_timer(self):
        ...
        self.cam.set_timer(self.data["timer"])
        
    def set_re(self):
        self.cam.set_re(self.w.value(), self.h.value())
    
    def st(self):
        self.cam.start(self.ul.text(),self.w.value(),self.h.value())
